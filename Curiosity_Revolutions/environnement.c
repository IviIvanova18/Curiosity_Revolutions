...
erreur_terrain initialise_environnement(Environnement *envt,
                                        char *fichier_terrain, State_new *state,int *waterFlag) {
  erreur_terrain errt;
...
resultat_deplacement avancer_envt(Environnement *envt,State_new *state) {
  transition_etat(A,state);
...
void gauche_envt(Environnement *envt,State_new *state) { 
  transition_etat(G,state);
  
  tourner_a_gauche(&(envt->r)); }

/* Tourner le robot à droite */
void droite_envt(Environnement *envt,State_new * state) { 
  transition_etat(D,state);
 ...
 */
int mesure_envt(Environnement *envt, int d,State_new * state) {
  
  if(d==1){
    transition_etat(M1,state);
  }
  else if(d==3){
    transition_etat(M3,state);
  }
  else if(d==7){
    transition_etat(M7,state);
  }
