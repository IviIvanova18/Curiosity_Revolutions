...
resultat_inter exec_pas(Programme *prog, Environnement *envt,
                        etat_inter *etat,State_new *state,int *flag) {
  Commande c;
  ...
  case Avancer:
    ...
    break;
  case Gauche:
    gauche_envt(envt,state);
    etat->pc++;
    res_inter = OK_ROBOT;
    break;
  case Droite:
    droite_envt(envt,state);
    etat->pc++;
    res_inter = OK_ROBOT;
    break;
  case Mesure:
    if (est_vide(&(etat->stack))) {
      ...
      depiler(&(etat->stack));
      mes = mesure_envt(envt, param,state);
      //If the robot sees a water the flag is changing to 1
      if(mes == 1){ 
        *flag = 1;
      }
